...
export function ArticleJsonLd({ article, settings }: ArticleJsonLdProps) {
  ...
  const articleJsonLd: WithContext<Article> = {
    ...
    dateModified: new Date(article._updatedAt || new Date().toISOString()).toISOString(),
    url: articleUrl,
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': articleUrl,
...
export async function CombinedJsonLd({
  ...
  return (
    <>
      {includeWebsite && cleanSettings && <WebSiteJsonLd settings={cleanSettings} />}
      {includeOrganization && cleanSettings && <OrganizationJsonLd settings={cleanSettings} />}
    </>
  )
}
